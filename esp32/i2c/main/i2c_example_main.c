...
#define LM75A_SENSOR_ADDR		   0x4C		    /*!<LM75A slave address */
...
static esp_err_t i2c_example_master_sensor_test(i2c_port_t i2c_num, uint8_t* data_h, uint8_t* data_l)
{
    ...
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, LM75A_SENSOR_ADDR << 1 | WRITE_BIT, ACK_CHECK_EN);
    ...
    */
    ...
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, LM75A_SENSOR_ADDR << 1 | READ_BIT, ACK_CHECK_EN);
    ...
    printf("data_h: %02x\n", (int)*data_h);
    printf("data_l: %02x\n", (int)*data_l);
    sensor_data = (int)*data_h << 3; 
    sensor_lsb = ((int)*data_l & 0x70) >> 5; 
    sensor_data |= sensor_lsb; 
    double tempC = (double)sensor_data * 0.125;  
...
static void i2c_test_task(void* arg)
{
    ...
    while (1) {
        ...
        if(ret == ESP_ERR_TIMEOUT) {
            ...
            printf("*******************\n");
            printf("TASK[%d]  MASTER READ SENSOR( LM75A )\n", task_idx);
            printf("*******************\n");
            printf("data_h: %02x\n", sensor_data_h);
            printf("data_l: %02x\n", sensor_data_l);
