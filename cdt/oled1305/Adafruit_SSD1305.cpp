/*********************************************************************
This is a library for our Monochrome OLEDs based on SSD1305 drivers

  Pick one up today in the adafruit shop!
  ------> https://www.adafruit.com/products/2675

These displays use I2C or SPI to communicate

Adafruit invests time and resources providing this open source code, 
please support Adafruit and open-source hardware by purchasing 
products from Adafruit!

Written by Limor Fried/Ladyada  for Adafruit Industries.  
BSD license, check license.txt for more information
All text above, and the splash screen below must be included in any redistribution
*********************************************************************/


#define pgm_read_byte(addr) (*(const unsigned char *)(addr))
#define _BV(bit) \
	(1 << (bit))

#define XLevelL         0x00
#define XLevelH         0x10
#define XLevel  ((XLevelH&0x0F)*16+XLevelL)

#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <wiringPi.h>
#include <wiringPiSPI.h>
#include <unistd.h>
#include "Hardware.h"
#include "Adafruit_GFX.h"
#include "Adafruit_SSD1305.h"

#include "glcdfont.c"

// a 5x7 font table
extern const uint8_t PROGMEM font[];

// the memory buffer for the LCD

static uint8_t buffer[SSD1305_LCDHEIGHT * SSD1305_LCDWIDTH / 8] = { 
,
//#if (SSD1305_LCDHEIGHT == 64)

};

Adafruit_SSD1305::~Adafruit_SSD1305() {

}



// the most basic function, set a single pixel
void Adafruit_SSD1305::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if ((x >= width()) || (y >= height()) || (x < 0) || (y < 0))
    return;

  // check rotation, move pixel around if necessary
  switch (getRotation()) {
  case 1:
    adagfx_swap(x, y);
    x = WIDTH - x - 1;
    break;
  case 2:
    x = WIDTH - x - 1;
    y = HEIGHT - y - 1;
    break;
  case 3:
    adagfx_swap(x, y);
    y = HEIGHT - y - 1;
    break;
  }  

  // x is which column
  if (color == WHITE) 
    buffer[x+ (y/8)*SSD1305_LCDWIDTH] |= _BV((y%8));  
  else
    buffer[x+ (y/8)*SSD1305_LCDWIDTH] &= ~_BV((y%8)); 
}

void Adafruit_SSD1305::initDisplay() {
  
  digitalWrite(RESETLINE, HIGH);
  	digitalWrite(RESETLINE, LOW);
  	usleep(15000);
  	digitalWrite(RESETLINE, HIGH);

  	usleep(1000);

  // Init sequence for 128x64 OLED module
  hdwObj.sendByte(COMMAND, SSD1305_DISPLAYOFF);                    // 0xAE
  hdwObj.sendByte(COMMAND, SSD1305_SETLOWCOLUMN | 0x4);  // low col = 0
  hdwObj.sendByte(COMMAND, SSD1305_SETHIGHCOLUMN | 0x4);  // hi col = 0
  hdwObj.sendByte(COMMAND, SSD1305_SETSTARTLINE | 0x0); // line #0
  hdwObj.sendByte(COMMAND, 0x2E); //??
  hdwObj.sendByte(COMMAND, SSD1305_SETCONTRAST);                   // 0x81
  hdwObj.sendByte(COMMAND, 0xFF);

  hdwObj.sendByte(COMMAND, SSD1305_SETBRIGHTNESS);                 // 0x82
  hdwObj.sendByte(COMMAND, 0x80);
  hdwObj.sendByte(COMMAND, SSD1305_SEGREMAP | 0x01);
  hdwObj.sendByte(COMMAND, SSD1305_NORMALDISPLAY);                 // 0xA6
  hdwObj.sendByte(COMMAND, SSD1305_SETMULTIPLEX);                  // 0xA8
  hdwObj.sendByte(COMMAND, 0x3F); // 1/64
  hdwObj.sendByte(COMMAND, SSD1305_MASTERCONFIG);
  hdwObj.sendByte(COMMAND, 0x8e); /* external vcc supply */
  hdwObj.sendByte(COMMAND, SSD1305_COMSCANDEC);
  hdwObj.sendByte(COMMAND, SSD1305_SETDISPLAYOFFSET);              // 0xD3
  hdwObj.sendByte(COMMAND, 0x40);
  hdwObj.sendByte(COMMAND, SSD1305_SETDISPLAYCLOCKDIV);            // 0xD5
  hdwObj.sendByte(COMMAND, 0xf0);
  hdwObj.sendByte(COMMAND, SSD1305_SETAREACOLOR);
  hdwObj.sendByte(COMMAND, 0x05);
  hdwObj.sendByte(COMMAND, SSD1305_SETPRECHARGE);                  // 0xd9
  hdwObj.sendByte(COMMAND, 0xF1);
  hdwObj.sendByte(COMMAND, SSD1305_SETCOMPINS);                    // 0xDA
  hdwObj.sendByte(COMMAND, 0x12);

  hdwObj.sendByte(COMMAND, SSD1305_SETLUT);
  hdwObj.sendByte(COMMAND, 0x3F);
  hdwObj.sendByte(COMMAND, 0x3F);
  hdwObj.sendByte(COMMAND, 0x3F);
  hdwObj.sendByte(COMMAND, 0x3F);
  hdwObj.sendByte(COMMAND, SSD1305_DISPLAYON);//--turn on oled panel
}


void Adafruit_SSD1305::invertDisplay(uint8_t i) {
  if (i) {
    hdwObj.sendByte(COMMAND, SSD1305_INVERTDISPLAY);
  } else {
    hdwObj.sendByte(COMMAND, SSD1305_NORMALDISPLAY);
  }
}



void Adafruit_SSD1305::data(uint8_t c) {

    hdwObj.sendByte(DATA,c);
}


void Adafruit_SSD1305::display(void) {
  uint16_t i=0;
  uint8_t page;
  if (SSD1305_LCDHEIGHT == 64) page = 0;
  if (SSD1305_LCDHEIGHT == 32) page = 4;

  for(; page<8; page++)
  {
    hdwObj.sendByte(COMMAND, SSD1305_SETPAGESTART + page);
    hdwObj.sendByte(COMMAND, 0x00);
    hdwObj.sendByte(COMMAND, 0x10);
      
      for(uint8_t x=0; x<128; x++) {
	hdwObj.sendByte(DATA, buffer[i++]);
      }
  }
}

// clear everything
void Adafruit_SSD1305::clearDisplay(void) {
  memset(buffer, 0, (SSD1305_LCDWIDTH*SSD1305_LCDHEIGHT/8));
}


void Adafruit_SSD1305::Set_Start_Column(unsigned char d) {
	hdwObj.sendByte(COMMAND, 0x00 + d % 16); // Set Lower Column Start Address for Page Addressing Mode
	//   Default => 0x00
	hdwObj.sendByte(COMMAND, 0x10 + d / 16); // Set Higher Column Start Address for Page Addressing Mode
									  //   Default => 0x10
}

void Adafruit_SSD1305::init_Hardware(void) {
	int status;

	pinMode(RESETLINE, OUTPUT);
	pinMode(DCLINE, OUTPUT);
	status = wiringPiSPISetup(0, 1000000);
	if (status < 1) {
		perror("Error opening SPI");
	}

}


void Adafruit_SSD1305::displayBuffer(void) {
	unsigned char *picture;
	unsigned char i, j;
	picture = buffer;

	for (i = 0; i < 8; i++) {
		//Set_Start_Page(i);
		hdwObj.sendByte(COMMAND, 0xB0 | i);
		Adafruit_SSD1305::Set_Start_Column(XLevel);
		for (j = 0; j < 0x84; j++) {
			hdwObj.sendByte(DATA, *picture);
			picture++;
		}
	}
}


int main() {

	Adafruit_SSD1305 oledObj;

	wiringPiSetup();
	oledObj.init_Hardware();
	oledObj.initDisplay();
	oledObj.displayBuffer();
	sleep(2);
	oledObj.clearDisplay();

	sleep(1);
	// oledObj.drawFastHLine(0,0,132,1);
	// oledObj.drawFastVLine(130,0,63,1);
	oledObj.drawFastVLine(2,0,63,1);
    oledObj.drawPixel(129,0,WHITE);
    oledObj.drawPixel(2,0,WHITE);
    oledObj.drawRect(2,0,128,64,WHITE);
    oledObj.drawChar(20,20,'f',WHITE, BLACK, 1);
    oledObj.drawChar(40,20,'F',WHITE, BLACK, 4);
    oledObj.displayBuffer();
    puts("Started");

	return 0;
}
