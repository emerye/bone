 ...
 */
void writeStringErase(int16_t x, int16_t y, int size, const char *string, unsigned short foreColor, unsigned short backColor, int yOffset) {
	...
	clsObj->getTextBounds((char *)string, x, y,
	       &x1,  &y1,  &w,  &h);
	clsObj->fillRect((unsigned int)x, (unsigned int)y1-yOffset, (unsigned int) w + (x1-x), (unsigned int) h, (unsigned int)backColor);
	clsObj->writeString(x,y,size,string, foreColor);
...
int main(int argc, char * argv[]) {
	...
	rpiObj.writeStringErase (x,100,1,"HIGHHFDHHD", RED, BLACK, 0);
//	rpiObj.writeString(x,50,1,"01234.567", WHITE);

	rpiObj.writeStringErase(x,100,1,"012.34567", GREEN, BLACK, 1);
...
//	writeStringErase (x,200,1,"HIGHHFDHHD", RED, WHITE);
