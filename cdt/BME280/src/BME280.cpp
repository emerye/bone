...
void Adafruit_BME280::write8(unsigned char reg, unsigned char value) {
	...
	retVal = wiringPiI2CWriteReg8(i2cHandle, reg, value);
	if (retVal == -1) {
...
uint32_t Adafruit_BME280::read20(unsigned char reg) {
	uint32_t value;
	int32_t lowByte;
	uint32_t highBytes;

	uint32_t msb, lsb, xlsb, result;
	...
	result = msb << 12 | lsb << 4 | ((xlsb >> 4) & 0x0F);
	// result = msb << 16 | lsb << 8 | xlsb;
	return result;
}
...
float Adafruit_BME280::readTemperature(void) {
	...
	int32_t adc_T = read20(BME280_REGISTER_TEMPDATA);
...
float Adafruit_BME280::readPressure(void) {
	...
	int32_t adc_P = read20(BME280_REGISTER_PRESSUREDATA);
	//adc_P >>= 4;
...
void Adafruit_BME280::initHardware(void) {
	int retVal;

	if ((retVal = wiringPiI2CSetup(BME280_ADDRESS)) == -1) {
			perror("Error opening I2C system");
		} else {
			sensorRef->setI2CHandle(retVal);
		}

		if (sensorRef->read8(BME280_REGISTER_CHIPID) != 0x60) {
			printf("Reading chip ID failed.\n");
		}
		sensorRef->readCoefficients();

			//Set before CONTROL_meas (DS 5.4.3)
		sensorRef->write8(BME280_REGISTER_CONTROLHUMID, 0x05); //16x oversampling

		sensorRef->write8(BME280_REGISTER_CONTROL, 0xB7); // 16x oversampling, normal mode
}


int main(int args, char *argv[]) {

	float temperature, pressure, humidity, altitude, temperatureF;
	...
	value = sensor.read16(0x89);
	printf("Read 16:  %04x\n", value);

	value = sensor.read16_LE(0x89);
	printf("Read 16LE:  %04x\n", value);

	value = sensor.read20(0x89);
	printf("Read 24:  %06x\n", value);
