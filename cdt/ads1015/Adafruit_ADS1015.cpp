...
uint8_t Adafruit_ADS1015::i2cread(void) {
	...
	result = wiringPiI2CRead(i2cHandle);
	return (result & 0xFF);
...
*/
/**************************************************************************/
void Adafruit_ADS1015::i2cwrite(uint8_t data) {
    int status;

	status = wiringPiI2CWrite(i2cHandle, data) ;
    if (status > 0) {
...
void Adafruit_ADS1015::writeRegister(uint8_t reg, uint16_t value) {
	...
	data = (value & 0xFF) << 8;
	data |= (value & 0xFF00) >> 8;
	wiringPiI2CWriteReg16 (i2cHandle, reg, data);
}
...
uint16_t Adafruit_ADS1015::readRegister(uint8_t reg) {
  ...
  usleep(50000);
  dataRead = wiringPiI2CReadReg16 (i2cHandle, reg);
  return (((dataRead & 0xFF) << 8) | (dataRead & 0xFF) );
}
...
*/
/**************************************************************************/
Adafruit_ADS1015::Adafruit_ADS1015(int i2cHandle)
{
...
int main(int argc, char **argv) {
	...
	wiringPiSetup();
	i2cfd = wiringPiI2CSetup(ADS1015_ADDRESS);
	if (i2cfd < 0) {
		perror("Error opening I2C handle for ADC1015.\n");
		return -1;
	}
