//TFT 320QUT with parallel interface. 
//

#define CAL_ADC_15T30	(*((unsigned int *) (0x10DA + 8)))
#define CAL_ADC_15T85	(*((unsigned int *) (0x10DA + 10)))
...
unsigned volatile long int clocktick = 57; 
unsigned volatile char minutes = 59;
unsigned volatile char hours = 12; 
char sBuffer[10] = { 0 };
char tBuffer[10] = { 0 }; 
...
void main()
{
	...
	unsigned char btnState0 = 0;
	unsigned char btnState1 = 0, btnState2 = 0;
	unsigned char btnLastState0 = 0, btnLastState1 = 0, btnLastState2 = 0;
	...
	TFT_Fill(BLACK);
 	...
 	TFT_Text("Temp", 0, 250, 16, BLUE, BLACK); 
}
...
* @param x1 
*/
void CreateButton(WORD x1, WORD y1, WORD x2, WORD y2, WORD border, WORD backcolor, char *btnText)
{
	BYTE count = 0; 
...
{
	Write_Command_Data(0x0044,(PX2 << 8) + PX1 );  //Horizonal RAM Address Position
	Write_Command_Data(0x0045,PY1);      // Vertical RAM address start
	Write_Command_Data(0x0046,PY2);  // Vertical RAM address end
	Write_Command_Data(0x004E,PX1);      // Set GDDRAM X address
	Write_Command_Data(0x004F,PY1);  // Set GDDRAM Y address
	Write_Command(0x0022);
}
...
void TFT_Text(char * S, WORD x, WORD y, BYTE DimFont, WORD Fcolor, WORD Bcolor)
{
	BYTE length,k;
	WORD buffer[24] = { 0 } ;
	BYTE charcount = 0; 
...
void TFT_Text32(char * S, WORD x, WORD y, WORD Fcolor, WORD Bcolor)
{
	BYTE length,k;
	WORD buffer[10] = { 0 } ;
	BYTE charcount = 0; 
...
void TFT_32Char(char C1, unsigned int x, unsigned int y, unsigned int Fcolor, unsigned int Bcolor)
{
...
void TFT_Char(char C1, unsigned int x, unsigned int y, unsigned char DimFont, unsigned int Fcolor, unsigned int Bcolor)
{
	unsigned char *ptrFont; 
...
void TFT_H_Line(unsigned char x1, unsigned char x2 ,unsigned char y_pos, unsigned int color)
{
	unsigned int k;
...
void TFT_V_Line(unsigned int y1,unsigned int y2, unsigned char x_pos , unsigned int color)
{
	unsigned int k; 
	for (k = y1; k <= y2; k++)
	{
...
void TFT_Circle(WORD x, WORD y, BYTE radius, BYTE fill, WORD color)
{
	int a_,b_,P;
	a_ = 0;
	b_ = radius;
...
void Set_GDRAM_Address(int x1, int y1)
{
	Write_Command_Data(0x004E,x1);      // Set GDDRAM X address
	Write_Command_Data(0x004F,y1);  // Set GDDRAM Y address
	Write_Command(0x22); 	
}

void TestLargeFont(void)
{
	int yStart, iCount;
	char mydigit = '0';
